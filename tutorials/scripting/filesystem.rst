...
same source files and assets while collaborating. redot stores
...
The redot file system avoids using metadata files. Existing asset managers and VCSs
are better than anything we can implement, so redot tries its best to play along
...
The ``project.redot`` file is the project description file, and it is always found
at the root of the project. In fact, its location defines where the root is. This
is the first file that redot looks for when opening a project.

This file contains the project configuration in plain text, using the win.ini
format. Even an empty ``project.redot`` can function as a basic definition of
a blank project.

Path delimiter
--------------

redot only supports ``/`` as a path delimiter. This is done for
portability reasons. All operating systems support this, even Windows,
so a path such as ``C:\project\project.redot`` needs to be typed as
``C:/project/project.redot``.
...
``project.redot`` is located, so ``res://project.redot`` is always
valid).
...
To avoid this, do all your move, delete and rename operations from within redot, on
the FileSystem dock. Never move assets from outside redot, or dependencies will have
to be fixed manually (redot detects this and helps you fix them anyway, but why
go the hard route?).
...
working with redot. One fool-proof convention is to only allow lowercase
file and path names.
