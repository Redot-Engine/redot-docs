...
A script file contains multiple types that derives from ``redotObject`` with
...
redot requires scripts to have a unique path so every type must be defined on its
