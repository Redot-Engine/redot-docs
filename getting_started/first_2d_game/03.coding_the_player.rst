...
.. tip:: If you've never used vector math before, or need a refresher, you can
         see an explanation of vector usage in redot at :ref:`doc_vector_math`.
...
enemies yet! That's OK, because we're going to use redot's *signal*
...
redot will create a function with that exact name directly in script
...
.. warning::
    ...
    a bug currently prevents redot from doing so. You'll be sent to your external
...
.. Note:: Disabling the area's collision shape can cause an error if it happens
          ...
          ``set_deferred()`` tells redot to wait to disable the shape until it's
          safe to do so.
