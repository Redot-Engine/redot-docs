...
collisions with the floor. To do so, we can use redot's :ref:`group
...
``move_and_slide()``, redot makes the body move sometimes multiple times in a
...
.. note::
    ...
    When using C#, redot will create the appropriate events automatically for all Signals ending with `EventHandler`, see :ref:`C# Signals <doc_c_sharp_signals>`.
