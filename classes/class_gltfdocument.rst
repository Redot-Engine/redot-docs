...
.. XML source: https://github.com/redotengine/redot/tree/master/modules/gltf/doc_classes/GLTFDocument.xml.
...
Treat the redot scene's root node as the root node of the glTF file, and mark it as the single root node via the ``redot_single_root`` glTF extension. This will be parsed the same as :ref:`ROOT_NODE_MODE_KEEP_ROOT<class_GLTFDocument_constant_ROOT_NODE_MODE_KEEP_ROOT>` if the implementation does not support ``redot_single_root``.
