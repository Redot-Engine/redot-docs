...
.. XML source: https://github.com/redotengine/redot/tree/master/doc/classes/Node3D.xml.
...
\ **Note:** Be aware that "Spatial" nodes are now called "Node3D" starting with redot 4. Any redot 3.x references to "Spatial" nodes refer to "Node3D" in redot 4.
...
- `All 3D Demos <https://github.com/redotengine/redot-demo-projects/tree/master/3d>`__
...
\ **Note:** Mixed negative scales in 3D are not decomposable from the transformation matrix. Due to the way scale is represented with transformation matrices in redot, the scale values will either be all positive or all negative.
