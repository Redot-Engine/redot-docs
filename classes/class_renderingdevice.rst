...
On startup, Redot creates a global **RenderingDevice** which can be retrieved using :ref:`RenderingServer.get_rendering_device()<class_RenderingServer_method_get_rendering_device>`. This global **RenderingDevice** performs drawing to the screen.
...
\ **Note:** Adjacency is only useful with geometry shaders, which Redot does not expose.
...
During a GPU crash in dev or debug mode, Redot's error message will include ``"REFLECTION_PROBES"`` for added context as to when the crash occurred.
...
During a GPU crash in dev or debug mode, Redot's error message will include ``"SKY_PASS"`` for added context as to when the crash occurred.
...
During a GPU crash in dev or debug mode, Redot's error message will include ``"LIGHTMAPPER_PASS"`` for added context as to when the crash occurred.
...
During a GPU crash in dev or debug mode, Redot's error message will include ``"SHADOW_PASS_DIRECTIONAL"`` for added context as to when the crash occurred.
...
During a GPU crash in dev or debug mode, Redot's error message will include ``"SHADOW_PASS_CUBE"`` for added context as to when the crash occurred.
...
During a GPU crash in dev or debug mode, Redot's error message will include ``"OPAQUE_PASS"`` for added context as to when the crash occurred.
...
During a GPU crash in dev or debug mode, Redot's error message will include ``"ALPHA_PASS"`` for added context as to when the crash occurred.
...
During a GPU crash in dev or debug mode, Redot's error message will include ``"TRANSPARENT_PASS"`` for added context as to when the crash occurred.
...
During a GPU crash in dev or debug mode, Redot's error message will include ``"POST_PROCESSING_PASS"`` for added context as to when the crash occurred.
...
During a GPU crash in dev or debug mode, Redot's error message will include ``"BLIT_PASS"`` for added context as to when the crash occurred.
...
During a GPU crash in dev or debug mode, Redot's error message will include ``"UI_PASS"`` for added context as to when the crash occurred.
...
During a GPU crash in dev or debug mode, Redot's error message will include ``"DEBUG_PASS"`` for added context as to when the crash occurred.
...
Tells the GPU what compute pipeline to use when processing the compute list. If the shader has changed since the last time this function was called, Redot will unbind all descriptor sets and will re-bind them inside :ref:`compute_list_dispatch()<class_RenderingDevice_method_compute_list_dispatch>`.
...
This is only used by Vulkan in debug builds. Redot must also be started with the ``--extra-gpu-memory-tracking`` :doc:`command line argument <../tutorials/editor/command_line_tutorial>`.
