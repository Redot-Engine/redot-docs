...
When importing, this controls what type of :ref:`CollisionObject3D<class_CollisionObject3D>` node Redot should generate. Valid values are ``"static"``, ``"animatable"``, ``"character"``, ``"rigid"``, ``"vehicle"``, and ``"trigger"``.
